...
//Indicating the current drive mode with sound
  indicateMode(mixingIndex + 1);
...
//Resizing the input signals to 255
  ...
  throttle_timer++;
  aux_timer++;
  if(steering_timer >= 12000){ inB = 0;rc_steering_ticks = 0;}
  if(throttle_timer >= 12000){ inA = 0;rc_throttle_ticks = 0;}
